...
# detect object หลังจากที่ kcf ติดตามไม่ได้
# ----------------- def detect ------------------------------
def yolo_detect(frame, dx,dy, count_recu):
	...
	if len(track_idxs) > 0:
			...
			for i in track_idxs.flatten():
				...
				# Check class and check distance (prev - curr) less than dist cal from fps
...
def receive_info():
	...
	if len(data) < 1000:
		try:
			...
			print("ts_sock except")
			#receive_info()
			

		print("--------------------------------------------------x_uav", frame_info["lat"])
		mmlat = frame_info["lat"]
		mmlon = frame_info["lon"]
		mmpit = frame_info["pit"]
		mmyaw = frame_info["yaw"]
		mmroll = frame_info["roll"]
		mmhei = frame_info["hei"]
...
while True:
	...
	#buffer = None
	#frame = None
	while len(dataa) < payload_size:
		packet = client_socket.recv(4*1024)
		if not packet: break
		dataa+=packet
	packed_msg_size = dataa[:payload_size]
	dataa = dataa[payload_size:]
	msg_size = struct.unpack("Q",packed_msg_size)[0]
	while len(dataa) < msg_size:
		dataa += client_socket.recv(4*1024)
	frame_dataa = dataa[:msg_size]
	dataa  = dataa[msg_size:]
	frame = pickle.loads(frame_dataa)

	print("end client sock")
	#------------------
	data2, address = sock.recvfrom(max_length)
	print("hi sock")
	
	if len(data2) < 1000:
		print("len(data2)", len(data2))
		frame_info = pickle.loads(data2)
		x_uav = frame_info["lat"]
		print("--------------------------------------------------x_uav", x_uav)
		lat = frame_info["lat"]
		lon = frame_info["lon"]
		pit = frame_info["pit"]
	...
	if not nowtracking :
		...
		if len(idxs) > 0:
			...
			for i in idxs.flatten():
				...
				if select == False and nowtracking == False :#and classIDs[i]==0: # and classIDs[i]==2:
					color = [int(c) for c in COLORS[classIDs[i]]]
					cv2.rectangle(frame, (x, y), (x + w, y + h), color, 2)
					text = "{}: {:.4f}".format(LABELS[classIDs[i]],confidences[i])
					cv2.putText(frame, text, (x, y - 5),cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
	...
	if nowtracking:
		...
		if not success:
			...
			try:
				...
				# 		images = images.view(-1, seq_dim, input_dim).requires_grad_()
				# 		outputs = model(images)
				# 		mxx, predicted = torch.max(outputs.data, 1)
				# 		print('predicted: ', predicted)
				# 		if predicted == 1:
				# 			id_nearest = i
				# 			print('id_nearest lstm', track_idxs[i])
				# 		predict_lstm.append(mxx)
				...
				# 	print('predict_lstm',predict_lstm)
				# 	print('output_lstm_np',outputs.detach().numpy().max())

				
				# id_nearest = check_object(track_idxs, t_boxes, t_classIDs, dx, dy)
				# print('id_nearest', id_nearest)

				# กรณีไม่ใช้ LSTM
				id_nearest, track_idxs, t_boxes, t_confidences, t_classIDs= yolo_detect(frame, dx, dy,count_recu)

				while id_nearest ==0 and count_recu <6:
					frame = receive_frame()
					count_recu+=1
					yolo_detect(frame, dx,dy, count_recu)
				nowtracking = False
		...
		# Geolocation 
		# มาจากตำแหน่งของวัตถุในรูป (x,y,w,h) * ขนาดเฟรม? 
		# Must Change to Right-Hand Oriented Frrame
		pv = y*480
		pu = (640 - x-w)*640
		...
		print("get_pitch",ts.get_pitch())
		print("init pitch", ts.init_pit())
		print("main_pitch", math.degrees(pit))
