...
class ROLO_utils:
        ...
        def load_yolo_output_test(self, fold, batch_size, num_steps, id):
                ...
                for path in paths_batch:
                        ...
                        yolo_output = np.load(path)
                        #print(yolo_output)
                        #!2yolo_output= np.reshape(yolo_output, 4102)
                        yolo_output= np.reshape(yolo_output, 689520)
                        yolo_output_batch.append(yolo_output)
                yolo_output_batch= np.reshape(yolo_output_batch, [batch_size*num_steps, 689520])
                return yolo_output_batch
        ...
        def load_rolo_gt_test(self, path, batch_size, num_steps, id):
                lines= self.load_dataset_gt(path)
                offset= num_steps - 2  # offset is for prediction of the future = 1
                st= id # 0
                ed= id + batch_size*num_steps # 0 + 3
                batch_locations= []
                for id in range(st+offset, ed+offset, num_steps): # (1,4,3)
                        location= self.find_gt_location(lines, id)
                        batch_locations.append(location)
                return batch_locations

        def load_rolo_gt_test2(self, path, num_steps, id):
                lines= self.load_dataset_gt(path)
                batch_locations= []
                #for i in range(id, id+num_steps, 1): # (1,4,3)
                location= self.find_gt_location(lines, id-1)
                batch_locations.append(location)
                return batch_locations
...
def locations_normal(wid, ht, locations):
    ...
    locations[2] *= wid
    locations[3] *= ht
    locations= [locations[0] - locations[2]/2, locations[1] - locations[3]/2, locations[2], locations[3]] # is this rlly help?
    return locations

def locations_normal_rolo(locations): # for rolo
    wid *= 1
    ht *= 1
    locations[0] *= wid
    locations[1] *= ht
